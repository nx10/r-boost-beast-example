...
namespace rbeast
{
    class ServerTask : public later::BackgroundTask
    {
    public:
        ServerTask(const std::string &t_host, unsigned short t_port, int t_threads)
            : taskid(0), host(t_host), port(t_port), threads(t_threads)
        {
        }
        ServerTask(int t_taskid, net::io_context *t_pioc)
    ...
    ServerTask *server = nullptr;
} // namespace rbeast


//' @export
// [[Rcpp::export]]
void beast_http_async(std::string host = "127.0.0.1", int port = 1234, int threads = 4)
{
    if (rbeast::server == nullptr)
    {
        rbeast::server = new rbeast::ServerTask(host, port, threads);
        rbeast::server->begin();
